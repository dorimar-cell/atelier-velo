...
Каталог — единственный контракт между исходными фото и рантаймом. Его пишет `scripts/process_parts.py`, читает `src/main.js` и `src/assemble.js`. На ветке `feat/flame-wrap-complete` каталог тот же, что на `main`; меняется только то, как финальная тень рисуется в рантайме.
...
| `role` | точка вылета в `FROM` и семантика слоя |
| `z` | порядок отрисовки (`renderOrder`) и глубина `z * 0.0018` |
| `bbox` | `[x, y, width, height]` в пикселях холста 2048×1366 |

Тень — отдельный объект `catalog.shadow`, не входит в `options` и не занимает шаг сборки. Появляется вместе с первой рамой. Её bbox `[195, 1194, 1668, 80]` задаёт и фото-овал, и якорь Flame Wrap.
...
На этой ветке нет таблиц `MIDPLANE` / `JOIN`. `assemble.js` ставит слой так:
...
Типичные `z` в каталоге:

| z | Слой |
| --- | --- |
| 1 | тень (и якорь Flame Wrap) |
...
3. Если это новый шаг — добавьте объект в `TYPES` и точку вылета в `FROM` (`assemble.js`).
